...
void completar_jugadas(const struct ataque *ataque, void *datos)
{
	...
	strcpy(datos_->jugadas_usuario[datos_->cantidad_jugadas].ataque, ataque->nombre);
...
void cargar_jugadas(datos_comprimidos_t* datos)
{
	...
	for (size_t i = 0; i < 3; i++) {
		pokemon_t *poke =lista_elemento_en_posicion(datos->pokes_usuario, i);
		for (int j = 0; j < 3; j++) {
			strcpy(datos->jugadas_usuario[pos_pokemon].pokemon, pokemon_nombre(poke));
			pos_pokemon++;
		}
...
{
	for (int i = 0; i < 9; i++) {
		if (strcmp(datos->jugadas_usuario[i].pokemon,
			   jugada.pokemon) == 0 &&
		    strcmp(datos->jugadas_usuario[i].ataque,
			   jugada.ataque) == 0) {
...
COMANDO_ESTADO mostrar_equipo_pokemon(void* contexto, void* lista_pokemones_usuario){
	...
	size_t cantidad = lista_con_cada_elemento(lista_pokemones, mostrar_pokemon, NULL);
	if(cantidad == 0){
		return SIN_POKES;
	}
	return ESTADO_OK;
...
COMANDO_ESTADO mostrar_equipo_pokemon_adversario(void* contexto, void* lista_pokemones_adversario){
	...
	size_t cantidad = lista_con_cada_elemento(lista_pokemones, mostrar_pokemon_adversario, NULL);
	if(cantidad == 0){
		return SIN_POKES;
	}
	return ESTADO_OK;
...
{	
	...
	menu_agregar_comando(menu, "Salir del programa", "s", "Sale del programa", salir_del_programa, &salir);
