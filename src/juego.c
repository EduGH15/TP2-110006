...
JUEGO_ESTADO juego_cargar_pokemon(juego_t *juego, char *archivo)
{
	...
	con_cada_pokemon(juego->info_pokemon, agregar_pokemon_a_lista,
			 juego->pokemones_totales);
	return TODO_OK;
}
...
int juego_obtener_puntaje(juego_t *juego, JUGADOR jugador)
{
	...
	return (jugador == JUGADOR1) ? juego->jugador_1->puntaje :
				       ...
				       const char *nombre3)
{
	...
	if (pokemon1 == pokemon2 || pokemon1 == pokemon3 ||
	    pokemon2 == pokemon3) {
		return POKEMON_REPETIDO;
	}

	if (jugador == JUGADOR1) {
		agregar_pokemon_a_jugador(juego->jugador_1, pokemon1, pokemon2,
					  NULL);
		agregar_pokemon_a_jugador(juego->jugador_2, NULL, NULL,
					  pokemon3);
	} else {
		agregar_pokemon_a_jugador(juego->jugador_1, NULL, NULL,
					  pokemon3);
		agregar_pokemon_a_jugador(juego->jugador_2, pokemon1, pokemon2,
					  NULL);
	}
	return TODO_OK;
}
				     ...
				     jugada_t jugada_jugador2)
{
	...
	pokemon_t *pokemon_jugador1 = lista_buscar_elemento(
		juego->jugador_1->pokemones, comparador_buscar_pokemon,
		jugada_jugador1.pokemon);
	pokemon_t *pokemon_jugador2 = lista_buscar_elemento(
		juego->jugador_2->pokemones, comparador_buscar_pokemon,
	...
	if (ataque_ya_utilizado(juego->jugador_1, ataque_jugador1) ||
	    ataque_ya_utilizado(juego->jugador_2, ataque_jugador2)) {
		return resultado_ataque;
	}

	int puntaje_jugador1 = calcular_efectividad_y_puntaje(
		ataque_jugador1, pokemon_jugador2, &resultado_ataque, juego,
		juego->jugador_1);
	int puntaje_jugador2 = calcular_efectividad_y_puntaje(
		ataque_jugador2, pokemon_jugador1, &resultado_ataque, juego,
		juego->jugador_2);
